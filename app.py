            ...
            with st.spinner("正在计算..."):
                ...
                for code in code_list:
                    try:
                        ...
                        mom = (hist['收盘'].iloc[-1] - hist['收盘'].iloc[-20]) / hist['收盘'].iloc[-20] * 100
                        data_list.append({"代码": code, "20日动量(%)": round(mom, 2)})
                    except: continue
                st.dataframe(pd.DataFrame(data_list).sort_values(by="20日动量(%)", ascending=False))
    with col2:
        if st.button("计算25日动量排序"):
            with st.spinner("正在计算..."):
                ...
                for code in code_list:
                    try:
                        hist = ak.fund_etf_hist_em(symbol=code, period="daily", start_date="20250101")
                        mom = (hist['收盘'].iloc[-1] - hist['收盘'].iloc[-25]) / hist['收盘'].iloc[-25] * 100
                        data_list.append({"代码": code, "25日动量(%)": round(mom, 2)})
                    except: continue
                st.dataframe(pd.DataFrame(data_list).sort_values(by="25日动量(%)", ascending=False))
